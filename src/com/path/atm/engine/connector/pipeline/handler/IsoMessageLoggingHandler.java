...
public class IsoMessageLoggingHandler extends LoggingHandler {
    ...
    static {
        try (InputStream stream = Thread.currentThread().getContextClassLoader()
        		.getResourceAsStream("com/path/simulator/iso/iso8583fields.properties")) {
    ...
    private int[] maskedFields;

    public IsoMessageLoggingHandler() {
        super(LogLevel.INFO);
        this.printSensitiveData = true;
        this.printFieldDescriptions = true;
    }
    ...
    protected String format(ChannelHandlerContext ctx, String eventName, Object arg) {
        if (arg instanceof AtmIsoMessage) {
        	StressSimulator.getInstance().getMainWindow().getConsolePanel()
        		.write(formatIsoMessage((AtmIsoMessage) arg));
            return super.format(ctx, eventName, formatIsoMessage((AtmIsoMessage) arg));
        } else {
            return super.format(ctx, eventName, arg);
        }
    }

    private String formatIsoMessage(AtmIsoMessage m) {
